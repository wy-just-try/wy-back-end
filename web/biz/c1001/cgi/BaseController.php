...
class BaseController extends \component\controller\BaseController
{
    ...
    public function renderJson($retcode = 0, Array $retdata = [])
    {
        ...
        $ret['errCode'] = $retcode;
        if(YII_DEBUG) {
            $ret['msg'] = C1001Errcode::getErrMsg($ret['errCode']);
        }
        if(!empty($retdata)) {
    ...
    public function renderFile($view, $params = [])
    {
        ...
        $params['retmsg'] = C1001Errcode::getErrMsg($retcode);
        //先转化为utf-8
